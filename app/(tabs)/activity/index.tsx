...
import { Text, View, ScrollView, Pressable, KeyboardAvoidingView, Platform, Switch, Alert } from 'react-native';
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      await createActivity({
        ...
        date_time: !isRecurring && date ? new Date(`${date} ${time}`).toISOString() : null,
      ...
      Alert.alert('Error', err.message ?? 'Could not post activity.');
    } finally {
      setSubmitting(false);
    }
  };
              ...
              >
                ...
                <Text
                  className={`text-sm font-medium capitalize ${
                    selectedCategory === cat.name ? 'text-primary-800' : 'text-neutral-700'
                  }`}
                >
                  {cat.name}
                </Text>
